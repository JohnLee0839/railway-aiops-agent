...
class Act(BaseModel):#模型输出两部分：action+new_steps
    """重新规划的输出格式"""
    action: str = Field(
...
async def replanner(state: PlanExecuteState) -> Dict[str, Any]:
    ...
    AIOPS-FUSION Recovery 模式:
    - 增加 RECOVERY_ATTEMPT 计数
    - 超过 MAX_RECOVERY_ATTEMPTS → 强制进入 Safety Control
    - 通过 recovery_result 标记最终结果
    """
    from app.models.incident import MAX_RECOVERY_ATTEMPTS
    ...
    if is_recovery:
        logger.info(f"[Recovery Mode] 当前尝试: {recovery_attempt + 1}/{MAX_RECOVERY_ATTEMPTS}")

    # ⚠️ AIOPS-FUSION: Recovery 模式最大尝试次数检查
    if is_recovery and recovery_attempt >= MAX_RECOVERY_ATTEMPTS:
        logger.error(
            f"[Recovery Mode] 恢复尝试次数已达上限 ({MAX_RECOVERY_ATTEMPTS})，"
            f"强制进入 Safety Control"
        )
        return {
            "response": (
                f"## 恢复失败 — 触发 Safety Control\n\n"
                f"已尝试 {recovery_attempt} 次恢复，均未成功。\n"
                f"系统自动进入安全控制层。\n\n"
                f"已执行的恢复步骤:\n" +
                "\n".join([f"- {step}" for step, _ in past_steps[-5:]])
            ),
    ...
    if plan:
        ...
        try:
            ...
            if action == "respond":
                ...
                # AIOPS-FUSION: Recovery 模式成功响应
                if is_recovery:
                    result["recovery_result"] = "recovery_success"
                return result
