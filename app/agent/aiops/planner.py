...
async def planner(state: PlanExecuteState) -> Dict[str, Any]:
    ...
    AIOPS-FUSION:
    - 正常模式: state["input"] 为用户任务描述
    - Recovery 模式: state["is_recovery"]=True, state["input"] 由 FailureContext.to_planner_input() 生成
    """
    logger.info("=== Planner：制定执行计划 ===")

    input_text = state.get("input", "")  # 从状态里拿到用户的问题。
    is_recovery = state.get("is_recovery", False)

    if is_recovery:
        recovery_attempt = state.get("recovery_attempt", 0)
        logger.info(f"[Recovery Mode] 恢复尝试 #{recovery_attempt + 1}, 基于 FailureContext 制定计划")
    logger.info(f"用户输入: {input_text[:200]}...")

    try:
        # 步骤1: 查询内部文档获取相关经验；如果找到，就放进 experience_docs
        ...
        planner_chain = planner_prompt | llm.with_structured_output(Plan)# 先把提示词交给模型，然后让模型按 Plan 这个格式输出
