...
class DocumentSplitterService:
    ...
    def __init__(self):
        """初始化文档分割服务"""
        self.chunk_size = config.chunk_max_size    # 默认 800 最终每个片段的目标大小
        self.chunk_overlap = config.chunk_overlap  # 默认 100 相邻片段之间的重叠字符数
        ...
        self.markdown_splitter = MarkdownHeaderTextSplitter(
            ...
            strip_headers=False,  # 确保标题文本本身仍保留在分片内容中，而不是被剔除，这样每个分片的上下文更完整
        )

        # 递归字符分割器 (用于二次分割，使用更大的chunk_size)
        self.text_splitter = RecursiveCharacterTextSplitter(
            chunk_size=self.chunk_size * 2,  # 二次分割器使用 两倍的 chunk_size（即 chunk_size * 2）作为目标大小
            chunk_overlap=self.chunk_overlap,
            length_function=len,
            is_separator_regex=False,  # 表示分隔符是普通字符串，而非正则
        )
    ...
    def split_markdown(self, content: str, file_path: str = "") -> List[Document]:
        ...
        """
        if not content or not content.strip(): # 空内容直接返回空列表，记录警告，不抛出异常（容错设计）
        ...
        try:
            ...
            # 第三阶段: 合并太小的分片 (< 300字符)
            final_docs = self._merge_small_chunks(docs_after_split, min_size=300) # 将小于 300 字符的小片段合并到前一个片段中，以避免产生过多无意义的小碎片。

            # 添加文件路径元数据 为每个文档的元数据添加 _source（源路径）、_extension 和文件名
        ...
        try:
            ...
            docs = self.text_splitter.create_documents(
                ...
                metadatas=[
                    {
                        "_source": file_path,
                        "_extension": Path(file_path).suffix, # 使用 Path(file_path).suffix 获取带点的扩展名（如 .txt）。
