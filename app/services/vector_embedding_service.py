...
class DashScopeEmbeddings(Embeddings):
    ...
    def __init__(
        ...
        # 这里没有设置 timeout、max_retries 等参数。在生产环境中，建议添加超时和重试配置，以增强鲁棒性。
        self.model = model
    ...
    def _mask_api_key(api_key: str) -> str:
        ...
        return "***"
    # 若 API Key 长度大于 8，显示前 8 位和后 4 位，中间用 ... 代替；否则全部显示为 ***。

    def embed_documents(self, texts: List[str]) -> List[List[float]]:#接收文本列表，返回对应的向量列表
        """
