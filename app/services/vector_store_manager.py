...
class VectorStoreManager:
    ...
    def _initialize_vector_store(self):
        ...
        try:
            ...
            connection_args = {
                "host": config.milvus_host, # 构造连接参数，从配置读取主机和端口。未配置用户名/密码、SSL 等，适用于简单内网环境。
                "port": config.milvus_port,
            }

            # 创建 LangChain Milvus VectorStore
            # 使用 biz collection，字段映射：text_field -> content, vector_field -> vector
            self.vector_store = Milvus( #实例化 langchain_milvus.Milvus，这是与 Milvus 交互的门面类
                embedding_function=vector_embedding_service,#传入嵌入服务实例，LangChain 在 add_documents 时会自动调用它生成向量。
                collection_name=self.collection_name,#指定集合名为 "biz"
