...
from langgraph.checkpoint.memory import MemorySaver # 导入内存检查点保存器，用于持久化会话状态（保存对话历史）。MemorySaver 将状态保存在内存中（重启后丢失），适合开发环境。
...
class AgentState(TypedDict):
    """Agent 状态"""
    messages: Annotated[Sequence[BaseMessage], add_messages] # dd_messages 告诉 LangGraph 当有新消息时如何合并（追加而非覆盖）
...
def trim_messages_middleware(state: AgentState) -> dict[str, Any] | None:
    ...
    # 如果消息数量不超过 7 条（包含系统消息 + 最多 3 轮用户+AI 对话），则不做裁剪。
    if len(messages) <= 7:
        return None
...
# 返回一个字典，包含 messages 字段。这里使用了 RemoveMessage 指令，先删除所有旧消息，再插入新消息。
...
class RagAgentService:
    ...
    def __init__(self, streaming: bool = True):
        ...
        self.model = ChatQwen(
            ...
            temperature=0.7,# 控制随机性，0.7 表示中等创造性
            streaming=streaming,
        )
    ...
    async def _initialize_agent(self):
        ...
        # 遍历配置中的 MCP 服务器，对每个服务器调用 suggest_mcp_transport 检查传输协议是否有误，如果有建议则记录警告。

        mcp_client = await get_mcp_client_with_retry()# 获取 MCP 客户端（带重试）
        ...
        # 安全加载 MCP 工具，如果失败则仅使用本地工具

        all_tools = self.tools + self.mcp_tools # 合并本地工具和 MCP 工具
    ...
    def _build_system_prompt(self) -> str:
        ...
        Returns:
            str: 系统提示词
        """
        from textwrap import dedent# 使用 dedent 移除缩进，构建一个清晰的中文系统提示词。这个提示词设定了 AI 的角色、工作原则和回答要求。
    ...
    async def query(
        ...
        """
        try:
            await self._initialize_agent()# 先确保 Agent 已初始化。
            ...
            # 调用 Agent（异步）。ainvoke 会执行整个图，直到结束，并返回最终状态

            # 从结果中提取最后一条消息的内容作为答案，并记录工具调用信息
    ...
    async def query_stream(
        ...
        question: str,
        session_id: str,
    ) -> AsyncGenerator[Dict[str, Any], None]:# 这是一个异步生成器，返回类型注解说明它产出字典。
        """
        流式处理用户问题（逐步返回答案片段）
    ...
    def get_session_history(self, session_id: str) -> list:
        ...
        try:
            ...
            # 遍历消息，跳过系统消息，将 HumanMessage 标记为 "user"，其他（AI）标记为 "assistant"，提取内容和时间戳。
