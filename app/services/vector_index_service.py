...
class VectorIndexService:
    ...
    def index_single_file(self, file_path: str):
        ...
        if not path.exists() or not path.is_file(): # 保路径存在且是文件（而非目录）
            raise ValueError(f"文件不存在: {file_path}")

        logger.info(f"开始索引文件: {path}")  # 记录 INFO 级别日志，方便追踪索引流程。使用 logger（通常是模块级或类属性）输出结构化信息。

        try:
            # 1. 读取文件内容 Path.read_text() 以 UTF-8 编码一次性读取全部内容到内存
            content = path.read_text(encoding="utf-8")
            logger.info(f"读取文件: {path}, 内容长度: {len(content)} 字符") # 日志记录：打印字符数，方便估算分割后的分片数量

            # 2. 删除该文件的旧数据（如果存在）
            normalized_path = path.as_posix()
            vector_store_manager.delete_by_source(normalized_path)   # 调用 vector_store_manager 删除该文件之前可能存在的所有向量分片

            # 3. 使用新的文档分割器
            documents = document_splitter_service.split_document(content, normalized_path)  # 调用 document_splitter_service将长文本按语义或固定长度切分为多个 Document 对象
            logger.info(f"文档分割完成: {file_path} -> {len(documents)} 个分片")

            # 4. 添加文档到向量存储
            if documents:
                vector_store_manager.add_documents(documents) # 调用 vector_store_manager.add_documents()，该方法内部会调用 Embedding 模型将每个分片转换为向量，并存入向量数据库
